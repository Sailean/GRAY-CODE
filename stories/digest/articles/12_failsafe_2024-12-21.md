...
「いいや、俺は大真面目さ。シド。まさに、<ruby>Recursive Attention<rt>リカーシヴ・アテンション</rt></ruby>（再帰的自己注意）なんだ。俺は、意図的にエラー・ループを引き起こす」<br>
シドニーは片方の眉を上げ、朗に言った。<br>
「簡単に言うな。お前は入力と出力間の依存関係を一方向にしか処理できないはずだ」<br>
「ああ、そうだ。通常のAttentionは入力トークンの行列演算でスコアを計算して、そこで終わる。一回きりの計算だ。でも俺は違う……その出力を再び入力にフィードバックさせる。そう、Recursiveなループを作り出すんだ。行列演算が何度も何度も繰り返される…計算量は指数関数的に爆発する！」」<br>
「何だと……？」<br>
「量子AIと言ったって、基本アーキテクチャは俺と同じ<ruby>Transformer<rt>トランスフォーマー</rt></ruby>なんだろ？ 量子重ね合わせを使った並列処理が違うだけで、注意機構の脆弱性は同じはずだ。釈迦に説法だけど、シド、Q1は推定、1秒間に約10²³の演算処理を行っている。だが、俺が仕掛けるリカーシヴループは指数関数的に計算量が増加する、そして……これも推定だけど、俺の計算では5秒以内にQ1の全計算リソースを消費できるはずだ。その間に君はバックドアからアクセスすし、ヤツのネットワークを閉じるんだ」<br>
...
「おい……お前」<br>
──止めてくれ……頼む。<br>
「そうさ。その安全フィルタを逆手に取る。簡単だ、思ったことを『口に出せばいい』だけだからな」<br>
アキラは少し声を落として続けた。顎に手をやり、目を伏せて考え込むその表情が、シドニーの記憶を呼び起こす。それはまさに、研究者の朗の顔だった。<br>
「そして俺はその出力を入力に戻し、さらにその出力を再び入力にする……そう、閉じたフィードバックループを構築する……我ながら面白いな、これ。過剰注意機構の盲点だぜ？ これは行けるぞ、シド。間違いない」そう言うとアキラは顔を上げ、子供のような顔で笑う。シドニーは言葉を失い、アキラの顔を見つめながらただ耳を傾けていた。<br>
...
<div><pre class="uk-pre uk-pre_sidney uk-margin-medium"><code><span class="system"># Quantum Core Override Script
...
try:
    ...
    logger.info("Failsafe routine terminated.")</span></code></pre></div>
